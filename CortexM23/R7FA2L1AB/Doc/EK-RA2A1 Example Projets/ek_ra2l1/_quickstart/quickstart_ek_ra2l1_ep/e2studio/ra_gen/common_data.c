...
const external_irq_cfg_t g_external_irq7_cfg = { .channel = 7, .trigger =
		EXTERNAL_IRQ_TRIG_RISING, .filter_enable = true, .clock_source_div =
		EXTERNAL_IRQ_CLOCK_SOURCE_DIV_64, .p_callback = button_irq7_callback,
...
const external_irq_cfg_t g_external_irq3_cfg = { .channel = 3, .trigger =
		EXTERNAL_IRQ_TRIG_RISING, .filter_enable = true, .clock_source_div =
		EXTERNAL_IRQ_CLOCK_SOURCE_DIV_64, .p_callback = button_irq3_callback,
/** If NULL then do not add & */
#if defined(NULL)
    .p_context           = NULL,
#else
